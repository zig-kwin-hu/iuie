...
class MOET5ForConditionalGeneration(T5PreTrainedModel):
    _keys_to_ignore_on_load_missing = [
        r"encoder.embed_tokens.weight",
        r"decoder.embed_tokens.weight",
        r"lm_head.weight",
    ...
    def forward(
        ...
        def mean_ignoring_padding(tensor, labels, ignore_index=-100, keepdim=False, keep_batch=False):
            if not isinstance(tensor, torch.Tensor):
                assert tensor == 0, "tensor should be 0, but is {}".format(tensor)
                return 0
            if keepdim:
                assert tensor.shape[:-1] == labels.shape, "tensor shape {} should be same as labels shape {}".format(tensor.shape, labels.shape)
            else:
            ...
            if keepdim:
                ...
                gate_num = masked_tensor.shape[-1]
                
                if keep_batch:
                    return masked_tensor.sum(dim = 1)/mask.sum(dim=1).unsqueeze(-1)
                else:
                    return masked_tensor.reshape(-1, gate_num).sum(dim = 0)/mask.sum()
            else:
                assert not keep_batch, "keep_batch should be False"
