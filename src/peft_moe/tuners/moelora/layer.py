...
class MOELoraLayer(BaseTunerLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            self.lora_A[adapter_name] = MOELinearA(self.in_features, r, self.expert_num, bias=False)
            self.lora_B[adapter_name] = MOELinearB(r, self.out_features, self.expert_num, bias=False, fan_in_fan_out=self.fan_in_fan_out)
    ...
    def reset_lora_parameters(self, adapter_name):
        if adapter_name in self.lora_A.keys():
            for i in range(self.expert_num):
                nn.init.kaiming_uniform_(self.lora_A[adapter_name].loraA[i].weight, a=math.sqrt(5))
                nn.init.zeros_(self.lora_B[adapter_name].loraB[i].weight)
        if adapter_name in self.lora_embedding_A.keys():
            raise NotImplementedError("reset_lora_parameters not implemented for embedding layer")
...
class MOEEmbedding(nn.Embedding, MOELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #not implemented
        raise NotImplementedError("MOEEmbedding not implemented")
        init_lora_weights = kwargs.pop("init_lora_weights", True)
...
class MOEConv2d(nn.Conv2d, MOELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        raise NotImplementedError("MOEConv2d not implemented")
        init_lora_weights = kwargs.pop("init_lora_weights", True)
