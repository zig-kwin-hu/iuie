...
from peft_moe.tuners.tuners_utils import BaseTuner, BaseTunerLayer, check_target_module_exists
from peft_moe.utils import (
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        **optional_kwargs,
        ...
        elif isinstance(target, MOELoraLayer) and isinstance(target, torch.nn.Embedding):
    ...
    def _create_new_module(lora_config, adapter_name, target, **kwargs):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = MOELinear8bitLt(adapter_name, target, **eightbit_kwargs)
            ...
            new_module = MOELinear4bit(adapter_name, target, **fourbit_kwargs)
        elif AutoGPTQQuantLinear is not None and isinstance(target, AutoGPTQQuantLinear):
            new_module = MOEQuantLinear(adapter_name, target, **kwargs)
            target.weight = target.qweight
        elif isinstance(target, torch.nn.Embedding):
            embedding_kwargs = kwargs.copy()
            embedding_kwargs.pop("fan_in_fan_out", None)
            in_features, out_features = target.num_embeddings, target.embedding_dim
            new_module = MOEEmbedding(adapter_name, in_features, out_features, **embedding_kwargs)
        elif isinstance(target, torch.nn.Conv2d):
            out_channels, in_channels = target.weight.size()[:2]
            kernel_size = target.weight.size()[2:]
            stride = target.stride
            padding = target.padding
            new_module = MOEConv2d(adapter_name, in_channels, out_channels, kernel_size, stride, padding, **kwargs)
        else:
    ...
    def _unload_and_optionally_merge(self, merge=True, progressbar: bool = False, safe_merge: bool = False):
        if merge:
            raise NotImplementedError("Merging is not supported for MOELoraModel yet.")
        ...
        for key in tqdm(key_list, disable=not progressbar, desc=desc):
            ...
            if isinstance(target, MOELoraLayer):
                ...
                elif is_bnb_available() and isinstance(target, MOELinear8bitLt):
                ...
                elif is_bnb_4bit_available() and isinstance(target, MOELinear4bit):
                    bias = target.base_layer.bias is not None
    ...
    def add_weighted_adapter(
        ...
        """
        raise NotImplementedError("Weighted adapter merging is not supported for LoraModel yet.")
    ...
    def _svd_weighted_adapter(
        ...
        conv2d = isinstance(target, MOEConv2d)
        if conv2d:
