...
def generate_random_cluster_embeddings(embedding_dim, cluster_num, dataset2id, target_dir):
    random.seed(0)
    np.random.seed(0)
    torch.manual_seed(0)
    embeddings = torch.nn.Linear(embedding_dim,cluster_num).weight.data.numpy()
    unique_id2index_cluster = {}
    for path in dataset2id:
        ...
        for set_name in ['train', 'dev', 'test']:
            ...
            with open('{}/{}.json'.format(path, set_name)) as fin:
                data = json.load(fin)
                for i in range(len(data)):
                    unique_id2index_cluster[data[i]['unique_id']] = dataset2id[path]
...
if __name__ == '__main__':
    ...
    dataset2id = {
        '{}/PolyglotNER_sample_20000'.format(prefix):0,
        '{}/Broad_Tweet_Corpus'.format(prefix):1,
        '{}/WikiANN_en'.format(prefix):2,
        '{}/WikiNeural_sample_20000'.format(prefix):3
    }
    target_dir = 'data/ie_instruct_unique_id/cluster_embeddings/'
    if not os.path.exists(target_dir):
        os.makedirs(target_dir)
    generate_random_cluster_embeddings(4096, 4, dataset2id, target_dir)
